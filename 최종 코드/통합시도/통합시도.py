...
bar_width, bar_height = 20, 120  # 캐릭터 가로,세로  설정   가로:20  세로:100
real_char = pygame.transform.scale(bar, (20, 180))  # 원본캐릭터에서에서 원하는 크기로 커스텀
xpos = 730 # 바의 x좌표위치
...
verdictBar= pygame.transform.scale(verdictBar,(60,180))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:  # 마우스가 눌렸는지
                if event.button == 4:  # 마우스휠 올리기
                    to_y = -6*bar_speed  # 캐릭터를 위로 캐릭터의 속도만큼 올리기
                if event.button == 5:  # 마우스휠내리기
                    to_y = +6*bar_speed  # 캐릭터를 아래로 캐릭터의 속도만큼 내리기 
            if event.type==pygame.KEYDOWN: #스페이스바를 누르면 judge함수 실행
                      judge()
